...
insert into public.company_leads (
  cnpj, legal_name, trade_name, registration_status, cnae_code, company_size,
  city, state, postal_code, address, source, relevance_score, raw_payload,
  search_alias, existing_client_id, existing_client_company_id
)
select distinct on (regexp_replace(coalesce(company.document, ''), '\D', '', 'g'))
  ...
  case upper(coalesce(company.size, '')) when 'M' then 'Médio' when 'P' then 'Pequeno' when 'G' then 'Grande' else nullif(trim(company.size), '') end,
