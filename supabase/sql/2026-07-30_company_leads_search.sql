-- Busca de leads (Clientes > Prospecção) com filtros, ordenação e paginação no banco.

create index if not exists company_leads_score_idx
  on public.company_leads (relevance_score desc, opened_at desc nulls last, id);
...
begin
  ...
  if cnae_value is not null then
    where_text := where_text || format(' and lead.cnae_code like %L', cnae_value || '%');
  end if;
