...
set
  legacy_created_at = case
    when coalesce(source_payload ->> 'created', '') ~ '^\d{4}-\d{2}-\d{2} '
      then (source_payload ->> 'created')::timestamp at time zone 'America/Sao_Paulo'
    else null
  end,
  legacy_updated_at = case
    when coalesce(source_payload ->> 'modified', '') ~ '^\d{4}-\d{2}-\d{2} '
      then (source_payload ->> 'modified')::timestamp at time zone 'America/Sao_Paulo'
    else null
  end
...
as $$
  select jsonb_build_object(
    'tickets', coalesce((
      ...
      ) order by coalesce(t.legacy_created_at, t.created_at) desc)
