...
function DriverSelect({ value, onChange, operators }: { value: string; onChange: (value: string) => void; operators: string[] }) { return <Field label="Motorista"><select className="h-10 w-full cursor-pointer rounded-md border border-input bg-background px-3 text-sm" value={value} onChange={(event) => onChange(event.target.value)}><option value="">Selecione um PRC</option>{operators.map((operator) => <option key={operator} value={operator}>{operator}</option>)}</select></Field>; }
